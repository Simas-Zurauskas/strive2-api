 ...
 * /api/recall/queue:
 *   get:
 *     summary: Get the user's daily recall queue (due items + fresh items, interleaved across courses)
 *     description: |
 ...
 *               properties:
 *                 data:
 *                   $ref: '#/components/schemas/RecallQueue'
 *       400:
 *         description: Invalid currentCourseId (not an ObjectId).
 */
export const getRecallQueueController = asyncHandler(async (req, res) => {
  ...
  const queue = await getRecallQueue({ userId, currentCourseId: currentCourseIdParam });
