...
const generateHeroImage = async ({
  ...
  try {
    ...
    if (await objectExists({ key: hashKey })) {
      genLog.info(`lesson:hero cache=hit hash=${hash.slice(0, 8)}…`);
      recordUsage({
        service: 'bfl',
    ...
    genLog.info(`lesson:hero cache=miss hash=${hash.slice(0, 8)}… — submitting to BFL`);
    ...
    genLog.info(`lesson:hero generated bytes=${buffer.byteLength} ms=${Date.now() - heroStart} s3=${hashKey}`);
    ...
    integrationLog.error(`bfl:gen fail reason=${reason}`);
    genLog.warn(`lesson:hero fail reason=${reason} — lesson ships without hero`);
    return null;
  }
};
...
export const imageGeneration = async (state: LessonState, config?: RunnableConfig): Promise<Partial<LessonState>> => {
  if (!state.includeImage) {
    genLog.info(`lesson:hero skip reason=user_opt_out`);
...
export const linksGeneration = async (state: LessonState, config?: RunnableConfig): Promise<Partial<LessonState>> => {
  if (!state.includeLinks) {
    genLog.info(`lesson:links skip reason=user_opt_out`);
