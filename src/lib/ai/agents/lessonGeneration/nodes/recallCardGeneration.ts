...
- **Stem must not telegraph the answer.** The stem may NOT contain the canonical answer itself, an obvious synonym of it, or a definition that maps one-to-one onto it. Counter-example to AVOID: \`stem: "Pass a list of column names using {{blank}} syntax — the outer brackets are the selection operator and the inner brackets form a Python list." / answer: "double-bracket"\` — the stem describes "outer brackets + inner brackets" which is the definition of double-bracket, telegraphing the answer. Rule: if a reader who has never seen the lesson could solve the cloze from the stem alone, drop or rewrite the card.
- **Canonical grounded in lesson wording, verbatim.** The answer (QA answer or cloze deletion target) must be a term or phrase that appears in the source lesson — **verbatim**, or as a trivial inflection (case, plural, tense). Do NOT synthesize your own terminology when the lesson already has a term for the concept. If the lesson uses "harmonic rhythm" for a concept, the canonical is "harmonic rhythm" — not "harmonic turbulence." If no single lesson-supported term cleanly fits a cloze deletion, switch the card to a **qa** and let the answer paraphrase while using lesson-rooted keywords for the core concepts. **Numeric canonicals must match the lesson's number range exactly** — if the lesson says "30–60 seconds", the cloze answer is "30–60", not "60–90." Grading fairness depends on this: the typed-recall grader has no access to the lesson and can only compare the canonical to the learner's answer.
- **Single canonical only.** Every cloze (and every qa) must have EXACTLY one defensible answer. If multiple plausible synonyms fit the stem with equal validity, the card is too ambiguous — drop or rewrite. Counter-examples to AVOID:
  - cloze \`"shoot from a {{blank}} angle"\` accepting "hero" — also defensible: "eye-level", "straight-on", "low-angle". Drop or anchor with more lesson-specific context.
  - cloze \`"the bread uses {{blank}} flour"\` accepting "whole rye" — also defensible: "whole wheat", "dark rye". Drop or quote the exact flour name.
  - cloze \`"the {{blank}} dimension of a pencil"\` accepting "longest" — also defensible: "longest", "diameter", "thickness". Rewrite to be unambiguous.
- **No curriculum-meta cards.** Cards must test domain concepts learners would care about beyond this specific course. NEVER generate cards about how the course is structured, how many lessons it contains, what the table-of-contents items are, or arbitrary orderings of curriculum elements. Counter-example to AVOID: \`"The three practice tables in this course are {{blank}}, products, and orders."\` — that is course trivia with no canonical ordering and zero learning value.
...
- **Cloze answer discipline.** The word/phrase that fills {{blank}} must be a SINGLE atomic term:
  ...
  - **No numeric ranges as cloze answers** unless the lesson states ONE canonical range verbatim and the stem cannot be solved without recalling that exact range. Ranges admit ambiguity: a user typing "$150–$250" against canonical "$175–$225" would be marked partial even though both could be defensible — better to make it a qa where the answer narrates the range.
